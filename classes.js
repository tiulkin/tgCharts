...
const navigationWindowResizeTrheshold = 20;
let theme = 'light';
...
class Diagram {
    ...
    updateCharts = animate => {
        ...
        if (animate) {
            Object.values(this.lines).forEach(line => {
                ...
                if (line.animateScale.beginElement) line.animateScale.beginElement();
                setTimeout(() => {
    ...
    createButtons = () => {
        Object.values(this.lines).forEach((line) => {
            ...
            checkmarkSVGCheck.classList.add('chart-button-svg-check');
    ...
    toggleLineVisibility = (e) => {
        ...
        if(!element.line.hidden) {
            element.classList.add('chart-button--off');
            setTimeout(() => element.classList.remove('chart-button--off'), 300);
        }
        setTimeout(() => element.classList.remove('chart-button--click'), 300);
        setTimeout(() => element.line.checkmarkSVGCheck.style.visibility = element.line.hidden ? 'hidden' : 'visible', 100);
    ...
    touchStartHandler = e => {
        this.toggleTooltip();
        let mode = 'move';
        const rect = this.navigationForegroundWindow.getBoundingClientRect();
        const touchX = (e.type === 'touchstart' ? e.touches[0].clientX : e.clientX);
        const touchY = (e.type === 'touchstart' ? e.touches[0].clientY : e.clientY);
    ...
    initEvents = () => {
        this.navigationBar.addEventListener('touchstart', this.touchStartHandler, false);
        this.navigationBar.addEventListener('mousedown', this.touchStartHandler, false);

        this.navigationBar.addEventListener('touchmove', this.touchMoveHandler, false);
        this.navigationBar.addEventListener('mousemove', this.touchMoveHandler, false);

        this.navigationBar.addEventListener('touchend', this.touchEndHandler, false);
        this.navigationBar.addEventListener('mouseup', this.touchEndHandler, false);
        this.navigationBar.addEventListener('mouseleave', this.touchEndHandler, false);
